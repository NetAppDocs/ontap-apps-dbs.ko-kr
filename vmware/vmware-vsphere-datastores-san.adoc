...
vSphere는 스토리지 장치에 대한 다중 경로를 기본적으로 지원합니다. vSphere는 지원되는 스토리지 시스템의 스토리지 장치 유형을 감지하고, 사용되는 프로토콜이나 ASA, AFF, FAS 또는 소프트웨어 정의 ONTAP 사용 여부에 관계없이 사용 중인 스토리지 시스템의 기능을 지원하도록 다중 경로 스택을 자동으로 구성할 수 있습니다.
...
NOTE: ESXi에서 지원되는 최신 제한 사항은 link:https://configmax.broadcom.com/guest?vmwareproduct=vSphere&release=vSphere%208.0&categories=2-0["VMware Configuration Maximums 툴"^]을(를) 참조하십시오.
...
* iSCSI 또는 NVMe/TCP 네트워크의 경우, 여러 가상 스위치가 있는 경우 NIC 팀 구성으로 서로 다른 라우팅되지 않은 네트워크 서브넷에 있는 여러 VMkernel 네트워크 인터페이스를 사용하십시오. 또한 여러 물리적 스위치에 연결된 여러 물리적 NIC를 사용하여 HA 및 처리량 향상을 제공할 수 있습니다. ONTAP에서 두 개 이상의 스위치에 연결된 두 개 이상의 링크로 구성된 단일 모드 인터페이스 그룹을 페일오버용으로 구성하거나, 다중 모드 인터페이스 그룹에서 LACP 또는 기타 링크 통합 기술을 사용하여 HA 및 링크 통합의 이점을 제공하십시오. NVMe/TCP는 VMkernel 포트별 및 물리적 인터페이스별로 활성화해야 하며, NVMe/TCP VMkernel 포트가 NVMe/TCP가 활성화된 물리적 NIC에 액세스할 수 있도록 정렬해야 합니다. 다음 그림은 IP 기반 SAN 프로토콜을 사용한 다중 경로 연결을 보여줍니다.image:vsphere_ontap_image2.png["다중 경로 연결"]
...
* vSphere 7.0에서 NVMe/FC를 사용하는 방법에 대한 자세한 내용은 이 문서 https://docs.netapp.com/us-en/ontap-sanhost/nvme_esxi_7.html["ONTAP NVMe/FC 호스트 구성 가이드"^] 및 https://www.netapp.com/pdf.html?item=/media/10681-tr4684.pdf["TR-4684를 참조하십시오"^]를 참조하십시오. 다음 그림은 vSphere 호스트에서 ONTAP LUN으로의 다중 경로 연결을 보여줍니다.
