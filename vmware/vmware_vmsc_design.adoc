...
ONTAP 툴 vCenter 플러그인은 vMSC용 SnapMirror 활성 동기화를 구성하는 간단하고 직관적인 방법을 제공합니다. ONTAP 툴 vCenter 플러그인을 사용하여 두 ONTAP 클러스터 간의 SnapMirror 활성 동기화 관계를 생성하고 관리할 수 있습니다. 이 플러그인은 이러한 관계를 효율적으로 설정하고 관리할 수 있는 사용하기 쉬운 인터페이스를 제공합니다. ONTAP 툴 vCenter 플러그인에 대한 자세한 내용은 https://docs.netapp.com/us-en/ontap-tools-vmware-vsphere-10/index.html["VMware vSphere용 ONTAP 툴"]에서 확인하거나 https://docs.netapp.com/us-en/ontap-tools-vmware-vsphere-10/protect/granular-datastore-protection-concept.html["ONTAP 툴의 세분화된 데이터스토어 보호 자격 기준"]으로 바로 이동하십시오.
