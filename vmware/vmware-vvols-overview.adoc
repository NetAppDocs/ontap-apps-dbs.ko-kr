...
VAAI는 스토리지로 오프로드되는 다양한 작업을 제공하지만, VASA Provider에서 해결하는 몇 가지 제한 사항이 있습니다. SAN VAAI는 VMware에서 관리하는 스냅샷을 스토리지 시스템으로 오프로드할 수 없습니다. NFS VAAI는 VM에서 관리하는 스냅샷을 오프로드할 수 있지만, 스토리지 네이티브 스냅샷을 사용하는 VM에는 제약이 있습니다. vVols는 가상 머신 디스크에 개별 LUN, 네임스페이스 또는 파일을 사용하므로, ONTAP은 파일이나 LUN을 빠르고 효율적으로 복제하여 델타 파일이 더 이상 필요하지 않은 VM 단위의 스냅샷을 생성할 수 있습니다. 또한 NFS VAAI는 핫(전원이 켜진) Storage vMotion 마이그레이션에 대한 복제 작업 오프로드를 지원하지 않습니다. 기존 NFS 데이터스토어와 함께 VAAI를 사용할 경우 마이그레이션 오프로드를 허용하려면 VM의 전원이 꺼져 있어야 합니다. ONTAP 툴의 VASA Provider는 핫 및 콜드 마이그레이션에 대해 거의 즉각적이고 스토리지 효율적인 복제를 지원하며, vVols의 볼륨 간 마이그레이션에 대한 거의 즉각적인 복사도 지원합니다. 이러한 상당한 스토리지 효율성 이점 덕분에 해당 https://www.netapp.com/pdf.html?item=/media/79014-ng-937-Efficiency-Guarantee-Customer-Flyer.pdf["효율성 보장"] 프로그램을 통해 vVols 워크로드를 최대한 활용할 수 있습니다. 마찬가지로, VAAI를 사용한 볼륨 간 복제가 요구 사항을 충족하지 못하는 경우, vVols의 향상된 복사 환경을 통해 비즈니스 과제를 해결할 수 있을 것입니다.
