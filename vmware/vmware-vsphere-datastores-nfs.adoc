...
* ONTAP 클러스터의 각 노드에서 각 SVM마다 하나의 논리 인터페이스(LIF)를 사용하십시오. 이전에 권장했던 데이터 저장소당 하나의 LIF 사용은 더 이상 필요하지 않습니다. 직접 액세스(LIF와 데이터 저장소가 동일한 노드에 있는 경우)가 가장 좋지만, 간접 액세스의 경우 성능에 미치는 영향이 일반적으로 미미하므로(마이크로초 단위) 걱정할 필요가 없습니다.
...
[TIP]
====
* vSphere 8에서 nconnect의 최대값은 4이며 기본값은 1입니다. vSphere의 최대값 제한은 고급 설정을 통해 호스트별로 높일 수 있지만 일반적으로 필요하지 않습니다.
* 단일 TCP 연결이 제공할 수 있는 것보다 더 높은 성능이 필요한 환경에는 4가 권장됩니다.
* ESXi는 NFS 연결 수를 256개로 제한하며, 각 nconnect 연결은 이 총 연결 수에 포함됩니다. 예를 들어 nconnect=4인 데이터스토어 두 개는 총 8개의 연결로 계산됩니다.
* 프로덕션 환경에 대규모 변경 사항을 구현하기 전에 사용자 환경에서 nconnect의 성능 영향을 테스트하는 것이 중요합니다.
...
* link:vmware-vsphere-settings.html["설정"]에서 언급한 바와 같이 Kubernetes용 vSphere CSI를 사용하지 않는 경우  https://knowledge.broadcom.com/external/article/386364/reducing-excessive-vsan-cnssync-warnings.html["VMware KB 386364"^]에 따라 newSyncInterval을 설정해야 합니다.
...
* ONTAP는 접합점을 사용하여 볼륨을 트리 구조로 구성할 수 있는 유연한 볼륨 네임스페이스 구조를 제공하지만, 이 방식은 vSphere에는 가치가 없습니다. 스토리지의 네임스페이스 계층 구조와 관계없이 데이터스토어의 루트에 각 VM에 대한 디렉터리가 생성됩니다. 따라서 vSphere용 볼륨의 접합 경로를 SVM의 루트 볼륨에 마운트하는 것이 가장 좋은 방법이며, 이것이 ONTAP tools for VMware vSphere가 데이터스토어를 프로비저닝하는 방식입니다. 접합 경로가 중첩되지 않으면 루트 볼륨 이외의 다른 볼륨에 종속되는 볼륨이 없으며, 의도적으로라도 볼륨을 오프라인으로 전환하거나 삭제하더라도 다른 볼륨에 대한 경로에는 영향을 미치지 않습니다.
