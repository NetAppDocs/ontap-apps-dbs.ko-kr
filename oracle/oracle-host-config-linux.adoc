...
(a) DNFS가 활성화되어 있고 (b) 소스 NFS 공유가 단일 서버에 두 번 이상 마운트되어 있으며 (c) 중첩 NFS 마운트가 있는 경우, `nosharecache`라는 추가 마운트 옵션이 필요합니다. 이러한 구성은 주로 SAP 애플리케이션을 지원하는 환경에서 볼 수 있습니다. 예를 들어, NetApp 시스템의 단일 NFS 공유에 `/vol/oracle/base`에 위치한 디렉터리와 `/vol/oracle/home`에 위치한 두 번째 디렉터리가 있을 수 있습니다.  `/vol/oracle/base`가 `/oracle`에 마운트되고 `/vol/oracle/home`가 `/oracle/home`에 마운트되면 동일한 소스에서 시작된 중첩 NFS 마운트가 생성됩니다.

OS는  `/oracle` 및  `/oracle/home`가 동일한 NFS 공유, 즉 동일한 소스 파일 시스템에 존재한다는 사실을 감지할 수 있습니다. 그런 다음 OS는 데이터를 액세스할 때 동일한 디바이스 핸들을 사용합니다. 이렇게 하면 OS 캐싱 및 특정 기타 작업의 사용이 향상되지만, DNFS에는 방해가 됩니다. DNFS가  `spfile`와 같은 파일을  `/oracle/home`에서 액세스해야 하는 경우, 잘못된 경로로 데이터를 액세스하려고 시도할 수 있습니다. 그 결과 I/O 작업이 실패하게 됩니다. 이러한 구성에서는 해당 호스트의 다른 NFS 파일 시스템과 소스 NFS 공유를 공유하는 모든 NFS 파일 시스템에  `nosharecache` 마운트 옵션을 추가하십시오. 이렇게 하면 Linux OS가 해당 파일 시스템에 대해 독립적인 디바이스 핸들을 할당하도록 강제할 수 있습니다.
