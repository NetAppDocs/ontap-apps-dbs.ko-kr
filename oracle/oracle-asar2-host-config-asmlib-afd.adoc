...
summary: ASA r2에서 Linux 및 ASMlib/ASM 필터 드라이버를 사용하는 Oracle 데이터베이스 
searchtitle: 'ASA r2에서 ASMLib/AFD(ASM 필터 드라이버)를 사용하는 Oracle 데이터베이스' 
---
...
[role="lead"]
ASA r2 ONTAP 에서 AFD 및 ASMlib를 사용하는 Linux OS 관련 구성 항목입니다.



== ASMlib 블록 크기

ASMlib은 선택적 ASM 관리 라이브러리 및 관련 유틸리티입니다. 이 기능의 가장 큰 장점은 LUN에 사람이 읽을 수 있는 레이블을 지정하여 ASM 리소스로 등록할 수 있다는 점입니다.
