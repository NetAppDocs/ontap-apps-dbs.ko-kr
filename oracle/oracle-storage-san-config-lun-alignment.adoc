...
로깅은 다양한 크기의 쓰기로 로그 파일의 순차적 쓰기를 수행합니다. 4KB 경계에 맞춰 정렬되지 않는 로그 쓰기 작업은 일반적으로 성능 문제를 야기하지 않는데 다음 로그 쓰기 작업에서 블록이 완료되기 때문입니다. 결과적으로 4KB 블록이 완료되면 ONTAP은 거의 모든 쓰기를 처리할 수 있으며, 이는 일부 4KB 블록의 데이터가 2개의 개별 작업으로 작성되었더라도 마찬가지입니다.
 ...
 `sio` 또는  `dd`와 같이 정의된 블록 크기로 I/O를 생성할 수 있는 유틸리티를 사용하여 정렬을 확인하십시오. 스토리지 시스템의 I/O 정렬 통계는  `stats` 명령을 사용하여 확인할 수 있습니다. 자세한 내용은 link:../oracle/oracle-notes-wafl-alignment-verification.html["WAFL 정렬 확인"]를 참조하십시오.

Solaris 환경에서의 정렬은 더 복잡합니다. 자세한 내용은 https://kb.netapp.com/on-prem/ontap/da/SAN/SAN-KBs/How_to_identify_unaligned_IO_on_LUNs["LUN에서 정렬되지 않은 IO를 식별하는 방법"]을 참조하십시오.
