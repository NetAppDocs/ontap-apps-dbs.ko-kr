...
가장 중요한 요구 사항은 데이터 파일을 하나 이상의 전용 컨테이너로 격리하는 것입니다. 이는 모든 데이터 파일을 단일 LUN/네임스페이스에 배치하거나, 단일 AFF NFS 볼륨 내에 배치하거나, 다중 볼륨 또는 다중 LUN/네임스페이스 ONTAP 일관성 그룹에 배치하는 등 여러 가지 방법으로 수행할 수 있습니다. 핵심은 데이터 파일이 다른 파일 유형과 혼합되지 않아야 한다는 것입니다. 이는 데이터 파일 복제가 아카이브 로그와 같은 다른 데이터 유형의 복제와 완전히 독립적으로 이루어지도록 하기 위함입니다. 파일 레이아웃 및 스토리지 레이아웃이 스냅샷 친화적인지 확인하는 데 필요한 중요한 세부 정보는 link:oracle-dp-online-backup.html#data-layout["데이터 레이아웃"]을 참조하십시오.
